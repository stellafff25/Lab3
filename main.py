...
class VHIApp(server.App):
    ...
    def parseRange(self, year_range):
    ...
    def getPlot(self, params):
        filtered_df = df[(df['area'] == int(params['area_key'])) &
                         (df['Year'].isin(self.parseRange(params['range_years'])))]

        week_range = params["range_weeks"]
        start_week, end_week = map(int, week_range.split('-'))

        filtered_df = filtered_df[(filtered_df['Week'] >= start_week) & (filtered_df['Week'] <= end_week)]

        ax = sns.lineplot(x='Year', y=params['index_key'], data=filtered_df, color='purple', marker='o')
    ...
    def getHTML(self, params):
        range_value = params["range"]
        return f"<h3>Введений інтервал місяців: {range_value}</h3>"
